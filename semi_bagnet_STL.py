...
class semi_bagnet_mnist_end_to_end(net_primal_mnist):  # Inherits modules from parent class
    ...
    def super_forward(self, x):
        x_prebag = self.prebag_network(x)
        M = self.prebag_mask.unsqueeze(0).unsqueeze(2).unsqueeze(3).repeat(x_prebag.shape[0], 1, x_prebag.shape[2],
                                                                           x_prebag.shape[3])
...
class semi_bagnet_mnist(net_primal_mnist):  # Inherits modules from parent class
    ...
    def clone_prebag_network(self, net_primal_instance):
        ...
        for i in range(len(self.prebag_network_trainable)):
            if str(self.prebag_network_trainable[i])[0:4] == 'Conv' or str(self.prebag_network_trainable[i])[
                                                                       0:4] == 'Batch':
                self.prebag_network_trainable[i].weight.data = copy(
                    net_primal_instance.prebag_network[i].weight.clone().data)
...
class three_way_semibagnet_end_to_end(nn.Module):
    ...
    def set_postbag_grad(self,value):

        for p in self.parameters():
            p.requires_grad = not(value)

        for p in self.postbag_network.parameters():
            p.requires_grad = value

        self.bnorm_fc.weight.requires_grad = value
        self.fc1.weight.requires_grad = value
        self.fc2.weight.requires_grad = value


    def copy_weights_from_parent(self, net_primal_instance):
        self.prebag_network = deepcopy(net_primal_instance.prebag_network)
        for p in self.prebag_network.parameters():
            p.requires_grad = True

        self.postbag_network = deepcopy(net_primal_instance.postbag_network)
        self.fc1 = deepcopy(net_primal_instance.fc1)
        self.fc2 = deepcopy(net_primal_instance.fc2)
        self.bnorm_fc = deepcopy(net_primal_instance.bnorm_fc)
