...
def train_semibag(net, trainloader, init_rate, step_size, gamma, total_epochs, weight_decay,mask):
    ...
    for epoch in range(total_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            total_loss = [0*sum(bagloss_items)/(bagouts.shape[2]*bagouts.shape[3]),criterion(outs, labels)]
...
def train_semibag_threeway(net, trainloader, init_rate, step_size, gamma, total_epochs, weight_decay):
    ...
    for epoch in range(int(total_epochs/2)):
...
def test_semibag_network(net, testloader, test_labels):
    ...
    for i in range(int(len(test_labels) / testloader.batch_size)):
        ...
        _, predicted = torch.max(outs+outs2, 1)
...
def create_traintestloaders(listdict):
    ...
    testloader = torch.utils.data.DataLoader(Data_test, batch_size=20, shuffle=False, num_workers=0)
...
if __name__ == "__main__":
    ...
    for idx in range(1):
        ...
        for i in range(val_splits):
            ...
            # networks = [net_primal_mnist(), semi_bagnet_mnist_end_to_end()]
            networks = [net_primal(in_channels=1),three_way_semibagnet_end_to_end()]

            # networks = [net_fullbag()]
            trainloader,trainloader_small,testloader = create_traintestloaders(listdict)
            ...
            torch.save(net,'./4conv_normal_mnist_10k_' + str(i) + '.pt')
            accuracy_train = test_network(net, trainloader_small, listdict[i]['train_label'])
            ...
            # networks[1].copy_weights_from_parent(copy(networks[0]))
            #
            # # for p in networks[1].prebag_network.parameters():
            # #     print(p.requires_grad)
            #
            # networks[2].prebag_mask = torch.from_numpy(np.float32(mask)).cuda()
            # networks[1].prebag_mask = mask.float()
            # net_semibag = train_semibag_threeway(networks[1], trainloader, init_rate, step_size, gamma, semibag_epochs, decay_normal)

            # net_semibag = torch.load('./vgg7_latest_semibag_threeway_end_to_end_stl10_training5000'+str(i)+'.pt')

            net_semibag = torch.load('./4conv_latestcorrect_semibag_threeway_end_to_end_FMNIST_training60k'+str(i)+'.pt')
            ...
            # net_semibag = torch.load('./4conv_Fmnist_60k_semibag_equalnetwork'+str(i)+'.pt')
            #
            #
            #
            # net_normal = torch.load('./4conv_normal_mnist_10k_' + str(i) + '.pt')
            # net_semibag = torch.load('./4conv_Fmnist_1k_semibag_ordersort_50'+str(i)+'.pt')


            accuracy_train = test_semibag_network(net_semibag, trainloader_small, listdict[i]['train_label'])
            accuracy = test_semibag_network(net_semibag, testloader, listdict[i]['test_label'])
